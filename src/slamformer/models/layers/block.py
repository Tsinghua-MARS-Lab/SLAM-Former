...
class BlockRope(nn.Module):
    ...
    def forward(self, x: Tensor, xpos=None, N=None, branch=1, global_=False, attn_mask=None, **kwargs) -> Tensor:
        def attn_residual_func(x: Tensor) -> Tensor:
            if attn_mask is not None:
                # Use externally provided mask directly
                _attn_mask = attn_mask
            elif global_:
                if branch == 1: # frontend
                    B,NP,C = x.shape
                    S = N
                    L = NP #S * P
                    P = int(NP//N)
                    frame_ids = torch.arange(L, device=x.device) // P  # [0,0,...,1,1,...,S-1]
                    future_frame = frame_ids.unsqueeze(1) < frame_ids.unsqueeze(0)
                    future_frame *= (frame_ids.unsqueeze(0)>1)
                    _attn_mask = future_frame.to(x.dtype) * torch.finfo(x.dtype).min 

                elif branch == 2: # backend
                    _attn_mask = None
                elif branch == 3: # mix-mode
                    B,NP,C = x.shape
                    S = N
                    L = NP #S * P
                    P = int(NP//N)
                    ...
                    frame_ids = torch.arange(L, device=x.device) // P  # [0,0,...,1,1,...,S-1]
                    future_frame = frame_ids.unsqueeze(1) < frame_ids.unsqueeze(0)
                    future_frame *= (frame_ids.unsqueeze(0)>=S_half)
                    _attn_mask = future_frame.to(x.dtype) * torch.finfo(x.dtype).min
            else:
                _attn_mask = None
            return self.ls1(self.attn(self.norm1(x), xpos=xpos, attn_mask=_attn_mask))

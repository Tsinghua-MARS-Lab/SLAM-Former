...
from dust3r.alignment import align_points_scale
...
def normalize_pointcloud(pts3d, valid_mask, eps=1e-3):
    ...
    valid_mask: B, H, W
    """
    #print('DEBUG',pts3d.shape, valid_mask.shape)
...
class CameraLoss(nn.Module):
    ...
    def forward(self, pred_pose, gt_pose):
        #print('SHAPE', pred_pose.shape, gt_pose.shape)
...
class DepthOrPmapLoss(nn.Module):
    ...
    def forward(self, pred, gt, sigma_p, sigma_g, valid_mask):
        if self.training:
            pred_normalized = pred #, _ = normalize_pointcloud(pred, valid_mask)
            gt_normalized = gt #, _ = normalize_pointcloud(gt, valid_mask)
        else:
        ...
        # Pi3 depth loss
        #main_loss = diff[valid_mask[...,None].expand(-1, -1, -1, C)] / (gt_normalized[...,-1][valid_mask]+1e-6) 
        #main_loss = main_loss.mean()

#(sigma[..., None].expand(-1, -1, -1, C) * diff)[valid_mask[..., None].expand(-1, -1, -1, C)].mean()
        #normal_loss = self.normal_loss(pred_aligned, gt_normalized, valid_mask)
        ...
        #return self.gamma * main_loss + normal_loss + reg_loss
...
class DistillLoss(MultiLoss):
    ...
    def compute_loss(self, gts, preds_,
                     track_queries=None, track_preds=None):
        # ---------- Lcamera ----------
        '''
        gt_extrinsics = ['camera_pose']
        gt_intrinsics = batch_data['intrinsics']
        image_hw = batch_data['images'].shape[-2:]
        '''

        _,h,w,_ = gts[0]['pts3d'].shape
        preds = preds_[0]
        #cam_gt = torch.stack([g['camera_pose'] for g in gts], dim=1)
        # use GT
        '''
        T12w = torch.inverse(gts[0]['camera_pose'])
        # Tw2i
        cam_gt_extrin = torch.stack([g['camera_pose']@T12w for g in gts], dim=1) # 1,8,4,4
        cam_gt_extrin = torch.inverse(cam_gt_extrin)
        '''
        cam_gt_extrin = torch.inverse(torch.stack([g['camera_pose'] for g in gts], dim=1)) # 1,8,4,4
        cam_gt_intrin = torch.stack([g['camera_intrinsics'] for g in gts], dim=1)
        cam_gt_pts3d_in_other_view = torch.stack([g['pts3d'] for g in gts],dim=1) # 1,8,H,W,3
        cam_gt_depth = torch.stack([g['depthmap'] for g in gts],dim=1) # 1,8,H,W,3


        B,S,h,w,_ = cam_gt_pts3d_in_other_view.shape
        cam_gt_pts3d_local = torch.stack([g['pts3d_local'] for g in gts],dim=1) # 1,8,H,W,3
#(cam_gt_extrin[:,:,:3,:3]@cam_gt_pts3d_in_other_view.view(B,S,-1,3).permute(0,1,3,2) + cam_gt_extrin[:,:,:3,3:]).permute(0,1,3,2).view(B,S,h,w,3)


        point_masks = torch.stack([g['valid_mask'] for g in gts],dim=1)
        #print('CHECK', point_masks.shape, cam_gt_pts3d_in_other_view.shape, cam_gt_depth.shape)
        cam_gt_extrin_34, cam_gt_pts3d_local, cam_gt_pts3d_in_other_view, cam_gt_depth = normalize_camera_extrinsics_and_points_batch(cam_gt_extrin[:,:,:3], cam_points=cam_gt_pts3d_local, world_points=cam_gt_pts3d_in_other_view, depths=cam_gt_depth, point_masks=point_masks)
        cam_gt_extrin[:,:,:3] = cam_gt_extrin_34

        for s in range(cam_gt_depth.shape[1]):
            gts[s]['depth'] = cam_gt_depth[:,s]
            gts[s]['pts3d_in_other_view'] = cam_gt_pts3d_in_other_view[:,s]
            gts[s]['pts3d_local'] = cam_gt_pts3d_local[:,s]


        # solve scales
        #closed_form_scale_and_shift(cam_gt_pts3d_local, preds['points'])
        pred_flat = preds['local_points'].reshape(-1,3)
        gt_flat = cam_gt_pts3d_local.reshape(-1,3)
        '''
        pred_mean = pred_flat.mean(0)
        gt_mean = gt_flat.mean(0)
        pred_centered = pred_flat - pred_mean
        ...
        scale = (pred_centered * gt_centered).sum() / (pred_centered ** 2).sum().clamp(min=1e-6)
        '''
        #weight_p = torch.ones(pred_flat.shape[0]).to(pred_flat.device)
        weight_p = point_masks.reshape(-1) 
        scale = align_points_scale(pred_flat, gt_flat, weight_p)
        ...
        cam_gt_extrin_relative = cam_gt_extrin[:,:,None] @ torch.inverse(cam_gt_extrin[:,None]) # 1,S,S,4,4
        cam_gt_intrin = cam_gt_intrin[:,None].repeat(1,S,1,1,1)

        #print('SHAPE', cam_gt_extrin_relative.shape, cam_gt_intrin.shape)
        #print('CHECK', cam_gt_extrin.shape, cam_gt_intrin.shape, gts[0]["true_shape"],h,w)
        cam_gt_ = extri_intri_to_pose_encoding(cam_gt_extrin_relative.view(B,S*S,4,4), cam_gt_intrin.view(B,S*S,3,3), (h,w),"absT_quaR_FoV")
        ...
        cam_pred_extrin, _ = pose_encoding_to_extri_intri(cam_pr, build_intrinsics=False) # 1,S,
        additional_row = torch.zeros(B, S, 1, 4).to(cam_pred_extrin)
        additional_row[..., -1] = 1  
        cam_pred_extrin = torch.cat([cam_pred_extrin,additional_row],axis=2)
        '''
        cam_pred_pose = preds['camera_poses'] 
        cam_pred_extrin = torch.inverse(cam_pred_pose) 

        #intrin = preds['intrin'] # B,S,2


        # relative
        cam_pred_extrin_relative = cam_pred_extrin[:,:,None] @ torch.inverse(cam_pred_extrin[:,None]) # 1,S,S,4,4
        cam_pr_ = extri_intri_to_pose_encoding(cam_pred_extrin_relative.view(B,S*S,4,4), cam_gt_intrin.view(B,S*S,3,3), (h,w),"absT_quaR_FoV") # 1,S*S,9
        #cam_pr_[:,:,7:] = cam_pr[:,:,7:][:,None].repeat(1,S,1,1).view(B,S*S,2)
        #intrin = intrin[:,:,None].repeat(1,1,S,1).view(B,S*S,2)
        ...
        #print('CHECK',cam_gt.shape, cam_pr.shape)
        #print('CHECK')
        #print(cam_pr)
        #print(cam_gt)
        Lcamera = self.cam_loss(cam_pr_, cam_gt_)

        #Ldepth = torch.zeros_like(Lcamera)
        # ---------- Ldepth ----------
        depth_terms = []
        for i,g in enumerate(gts):#, preds):
            if ('depth' in g):# and ('depth' in p):
                sigma_p = preds['conf'][:,i].squeeze(-1)
                sigma_g = None #g['conf']
                valid_mask = g['valid_mask']
                if not valid_mask.any():
                    valid_mask = torch.ones_like(g['valid_mask'])
 
                depth_terms.append(self.depth_loss(preds['local_points'][:,i,:,:,2:]*scale, g['depth'].view(preds['local_points'][:,i,:,:,2:].shape), sigma_p, sigma_g, valid_mask))

        Ldepth = torch.stack(depth_terms).mean() if depth_terms else torch.zeros_like(Lcamera)
        '''
        print('CHECK')
        print(preds[0]['pts3d_in_other_view'][:,100:-100,100:-100])
        print(gts[0]['pts3d_in_other_view'][:,100:-100:,100:-100])
        print("MAX", preds[0]['pts3d_in_other_view']/(gts[0]['pts3d_in_other_view']+1e-6))

        print("MAX", preds[0]['depth'].mean(), gts[0]['depth'].mean())
        print(preds[0]['depth'][:,:10])
        print(gts[0]['depth'][:,:10])
        '''

        # ---------- Lpmap ----------
        # transform pointmap to a frame
        world_frame = 0 #np.random.randint(S)
        Twc_pred = cam_pred_extrin_relative[:,world_frame] # 1,S,4,4
        Twc_gt = cam_gt_extrin_relative[:,world_frame]

        pred_p = (Twc_pred[:,:,:3,:3]@preds['local_points'].view(B,S,-1,3).permute(0,1,3,2) + Twc_pred[:,:,:3,3:]).permute(0,1,3,2).view(B,S,h,w,3) * scale
        gt_p = (Twc_gt[:,:,:3,:3]@cam_gt_pts3d_local.view(B,S,-1,3).permute(0,1,3,2) + Twc_gt[:,:,:3,3:]).permute(0,1,3,2).view(B,S,h,w,3)
        ...
        if ('conf' in preds):
            ...
            for i,g in enumerate(gts):#,preds):
                ...
                loss_pmap = self.pmap_loss(pred_p[:,i],
                                       gt_p[:,i],
                                       sigma_p,
                                       sigma_g,
                                       valid_mask)
                ...
                '''
                loss_pmap = self.pmap_loss(preds['local_points'][:,i]*scale,
                                       g['pts3d_local'],
                                       sigma_p,
                                       sigma_g,
                                       valid_mask)
                ''' 
                pmap_terms.append(loss_pmap)
        ...
        total = Lcamera * 20*100 + Ldepth * 20 + Lpmap * 10 + self.lambda_track * 10 * Ltrack
        #total = Lcamera * 20*10  + Lpmap * 10 + self.lambda_track * 10 * Ltrack
